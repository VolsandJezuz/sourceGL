#ifndef PLUGINS_NVIDIA_NVAPI_H_
#define PLUGINS_NVIDIA_NVAPI_H_

#define STRICT
#define WIN32_LEAN_AND_MEAN

//#include "NVIDIA.h" if needs to be exported, is currently std::is_pod valid
#include <vector>
#include <Windows.h>

namespace nvidia {

class NvAPI // PLUGIN_NVIDIA_API if needs to be exported, is currently std::is_pod valid
{
public:
	static const int NVAPI_MAX_PHYSICAL_GPUS = 64;
	static const int NVAPI_MAX_COOLER_PER_GPU = 20;
	...
	typedef struct
	{
		unsigned int version;
		unsigned int count;
		NV_COOLER Coolers[NVAPI_MAX_COOLER_PER_GPU];
	...
	{
		unsigned int version;
		NV_LEVEL Level[NVAPI_MAX_COOLER_PER_GPU];
	...
	typedef int (*NvAPI_EnumPhysicalGPUs_t)(int** nvGPUHandle, int* pGpuCount);
	typedef int (*NvAPI_GPU_GetFullName_t)(int* hPhysicalGpu, char szName[64]);
	...
	typedef int (*NvAPI_GPU_GetCoolerSettings_t)(int* hPhysicalGpu, int coolerIndex, NV_GPU_COOLER_SETTINGS* pCoolerInfo);
	...
	char* GetAssociatedNvidiaDisplayName(int nDisp, char* szDispName);
	int EnumPhysicalGPUs(int** nvGPUHandle);
	NV_DISPLAY_DVC_INFO_EX GetDVCInfoEx(int nDisp);
	NV_GPU_COOLER_SETTINGS GPU_GetCoolerSettings(int nGPU);
	char* GPU_GetFullName(int nGPU, char* szName);
	bool SetDVCInfoEx(int nDisp, int Level);
	bool SetCoolerLevels(int nGPU, int nCooler, int newLevel);
	void Stop() const;
	...
	NvAPI_GPU_SetCoolerLevels_t NvAPI_GPU_SetCoolerLevels;
	NvAPI_Unload_t NvAPI_Unload;
};

} // namespace nvidia

#endif // PLUGINS_NVIDIA_NVAPI_H_